...
def routes_stats(
    ...
    `total_m` считается по полной длине `route.geometry`. Длины по покрытиям
    считаются по сопоставлению сегментов маршрута с `reference_segment.geometry`.
    Если сегмент маршрута не сопоставлен с опорной сетью, его длина попадает в
    `other_m`.
